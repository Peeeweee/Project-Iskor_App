...
const MatchDetailModal: React.FC<MatchDetailModalProps> = ({ match, onClose }) => {
  ...
  const renderBoxScore = () => {
    ...
    const regularPeriods = Array.from({ length: match.periods }, (_, i) => i + 1);
    const overtimePeriods = match.overtimePeriods || [];
      ...
      <table className="w-full text-left mt-4">
        <thead>
          <tr className="border-b border-light-border dark:border-dark-border">
            ...
            {allPeriods.map(p => <th key={p} className="p-2 text-center">{typeof p === 'number' ? (sport === Sport.Basketball ? `Q${p}`: `P${p}`) : p}</th>)}
        ...
        <tbody>
          <tr className="border-b border-light-border dark:border-dark-border">
            ...
            {allPeriods.map((p, index) => <td key={p} className="p-2 text-center font-mono">{periodScores[index]?.a ?? '-'}</td>)}
          ...
          <tr>
            ...
            {allPeriods.map((p, index) => <td key={p} className="p-2 text-center font-mono">{periodScores[index]?.b ?? '-'}</td>)}
